...
public class ConnectionInfoMessageTest {
	...
	public void shouldReturnConnectionInfoFromJSON() {
		// final String jsonMessage = "{connectionInfo : { user : 'Miguel Arlandy',
		// activeUsers : ['Cristiano Ronaldo', 'Fernando Alonso', 'Rafael Nadal'] } }";
		// final ConnectionInfoMessage message = jsonProcessor.fromJson(jsonMessage,
		// ConnectionInfoMessage.class);
		...
		// message.getConnectionInfo().getActiveUsers().get(0));
		// assertEquals("Fernando Alonso",
		// message.getConnectionInfo().getActiveUsers().get(1));
		// assertEquals("Rafael Nadal",
		// message.getConnectionInfo().getActiveUsers().get(2));
	}

	@Test
	public void shouldReturnJSONFromConnectionInfo() {
		// final ConnectionInfoMessage connectionInfoMessage = new
		// ConnectionInfoMessage("Miguel Arlandy", Arrays.asList("Cristiano Ronaldo",
		// "Fernando Alonso", "Rafael Nadal"));
		// final String jsonMessage = jsonProcessor.toJson(connectionInfoMessage);
		// assertEquals("{\"connectionInfo\":{\"user\":\"Miguel
		// Arlandy\",\"activeUsers\":[\"Cristiano Ronaldo\",\"Fernando Alonso\",\"Rafael
		// Nadal\"]}}", jsonMessage);
	}
